...
parser.add_argument('-dropout', type=float, default=0.1, help="dropout [default=0.5]")
parser.add_argument('-lr', type=float, default=0.001, help="learning rate [default=0.001]")
parser.add_argument('-hidden-size', type=int, default=100, help="the size of the sentence representations")
...
class HierarchicalSent(nn.Module):
	...
	#print type(abs_offsets_tensor)
        ...
        for i in range(self.K):
            ...
            h.append(torch.nn.functional.relu(h_temp))
        
        dec_output = Variable(torch.zeros(self.K, num_batch, voc_size).type(dtype))
        for b in range(num_batch):
...
if args.optimizer == "SGD":
    optimizer = torch.optim.SGD(hs.parameters(), lr=lr, momentum=0.9)
...
for epoch in range(args.epochs):
    ...
    for b in range(batch_size):
        ...
        #image = Variable(Xtrain[x_ind, :].unsqueeze_(0)).cuda()
    sentence = Variable(Xtrain[x_ind, :]).cuda()
    neighbour = Variable(Xtrain[(x_ind + offsets), :]).cuda()
    if epoch == 10:
        res = hs.get_kth_sentence_encoding(sentence, 1)
        #print res.size()
    ...
    if epoch and epoch % 200 == 0:
        print epoch, train_loss_avg / 200.0
        train_loss_avg = 0
...
for K in range(1, args.K+2):
    ...
    hk = hs1.get_kth_sentence_encoding_numpy(Xtrain_np[0:5000], K-1)
